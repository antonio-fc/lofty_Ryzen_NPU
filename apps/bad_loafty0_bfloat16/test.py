...
def main(opts):
    ...
    INOUT3_DATATYPE = dtype

    INOUT0_SIZE = INOUT0_VOLUME * 2
    INOUT1_SIZE = INOUT1_VOLUME * 2
    INOUT2_SIZE = INOUT2_VOLUME * 2
    INOUT3_SIZE = INOUT3_VOLUME * 2

    OUT_SIZE = INOUT3_SIZE
    OUT_DATATYPE = INOUT3_DATATYPE
    ...
    inout1 = np.ones(INOUT0_VOLUME, dtype=INOUT0_DATATYPE)          # vis
    
    inout2a = np.ones(INOUT0_VOLUME, dtype=INOUT0_DATATYPE) # u
    inout2b = np.ones(INOUT0_VOLUME, dtype=INOUT0_DATATYPE) # v
    inout2c = np.ones(INOUT0_VOLUME, dtype=INOUT0_DATATYPE) # w
    ...
    # inout3[0::3] = l.flatten().astype(dtype)
    # inout3[1::3] = m.flatten().astype(dtype)
    # inout3[2::3] = n.flatten().astype(dtype)
    ...
    for i in range(num_iter):
        ...
        entire_buffer = bo_inout4.read(OUT_SIZE, 0).view(bfloat16)
