...
def loafty(opts):
    ...
    MIN_SIZE = 2 # the size of the frequency data points comming in the main input streams, how many pixels (lmn) are moved at once
    N_LMN = 3 # 3 for l, m and n
    ITERS = BSIZE//MIN_SIZE
    ...
    def device_body():
        ...
        join_main_move_ty = np.ndarray[(MAIN_MOVE_SIZE*NCHANNELS,), dtype]
        out_ty = np.ndarray[(MIN_SIZE,), dtype]
        ...
        # Inputs
        of_in_lmn = object_fifo("in0", st[2], mean_tile, 2, lmn_broad_ty)
        ...
        for c in range(NCHANNELS):
            ...
            w_add_fifos.append(object_fifo(f"of_add_w{c}", dist_tiles[c][4], add_tiles[c][1], 2, single_input_ty))
        ...
        # Send the real and imaginary of both channels to the respective Mem Tile for joining
        realJoinFifos = []
        imagJoinFifos = []
        join_offsets = [i*MAIN_MOVE_SIZE for i in range(NCHANNELS)]
        for c in range(NCHANNELS):
            realJoinFifos.append(object_fifo(f"of_realJoin{c}", real_tiles[c], memJoinR, 2, main_move_ty))
            imagJoinFifos.append(object_fifo(f"of_imagJoin{c}", imag_tiles[c], memJoinC, 2, main_move_ty))
        real_joined_fifo = object_fifo(f"of_real_joined", memJoinR, sub_tile, 2, join_main_move_ty)
        imag_joined_fifo = object_fifo(f"of_imag_joined", memJoinC, sub_tile, 2, join_main_move_ty)
        
        object_fifo_link(realJoinFifos, real_joined_fifo, join_offsets, [])
        object_fifo_link(imagJoinFifos, imag_joined_fifo, join_offsets, [])

        # Movment from sub_tile to mean_tile
        of_sub = object_fifo("sub", sub_tile, mean_tile, 2, join_main_move_ty)                
        ...
        for c in range(NCHANNELS):
            ...
            #                 for _ in range_(MIN_SIZE): # i determines whether to use the first (0) or second (1) set of lmn
            ...
            @core(scale_tiles[c][0]) # Scaling v by m
            def core_body():
                for _ in range_(ITER_KERNEL):
                    inputs = main_in_fifos[c][2].acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                    for _ in range_(BSIZE//MIN_SIZE): # img_size**2 / 2
                        # lmn = of_in_lmn.acquire(ObjectFifoPort.Consume, 1) # 6
                        for _ in range_(MIN_SIZE): # i determines whether to use the first (0) or second (1) set of lmn
                            output = bl_add_fifos[c][0].acquire(ObjectFifoPort.Produce, 1) # 4608
                            # run kernel here
                            bl_add_fifos[c][0].release(ObjectFifoPort.Produce, 1)
                        # of_in_lmn.acquire(ObjectFifoPort.Consume, 1)
                    main_in_fifos[c][2].release(ObjectFifoPort.Consume, 1)

            @core(scale_tiles[c][1]) # Scaling v by m
            def core_body():
                for _ in range_(ITER_KERNEL):
                    inputs = main_in_fifos[c][3].acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                    for _ in range_(BSIZE//MIN_SIZE): # img_size**2 / 2
                        # lmn = of_in_lmn.acquire(ObjectFifoPort.Consume, 1) # 6
                        for _ in range_(MIN_SIZE): # i determines whether to use the first (0) or second (1) set of lmn
                            output = bl_add_fifos[c][1].acquire(ObjectFifoPort.Produce, 1) # 4608
                            # run kernel here
                            bl_add_fifos[c][1].release(ObjectFifoPort.Produce, 1)
                        # of_in_lmn.acquire(ObjectFifoPort.Consume, 1)
                    main_in_fifos[c][3].release(ObjectFifoPort.Consume, 1)

            @core(scale_tiles[c][2]) # Scaling w by n
            def core_body():
                for _ in range_(ITER_KERNEL):
                    inputs = main_in_fifos[c][4].acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                    for _ in range_(BSIZE//MIN_SIZE): # img_size**2 / 2
                        # lmn = of_in_lmn.acquire(ObjectFifoPort.Consume, 1) # 6
                        for _ in range_(MIN_SIZE): # i determines whether to use the first (0) or second (1) set of lmn
                            output = bl_add_fifos[c][2].acquire(ObjectFifoPort.Produce, 1) # 4608
                            # run kernel here
                            bl_add_fifos[c][2].release(ObjectFifoPort.Produce, 1)
                        # of_in_lmn.acquire(ObjectFifoPort.Consume, 1)
                    main_in_fifos[c][4].release(ObjectFifoPort.Consume, 1)
            ...
            @core(add_tiles[c][1]) # Adding the sum u-v and w
            def core_body():
                for _ in range_(ITER_KERNEL):
                    for _ in range_(BSIZE):
            ...
            def core_body():
                for _ in range_(ITER_KERNEL):
                    visR = main_in_fifos[c][0].acquire(ObjectFifoPort.Consume, 1) # 4610
                    for _ in range_(BSIZE):
                        inputs = add2mainFIFOs[c].acquire(ObjectFifoPort.Consume, 1) # 4608
                        output = realJoinFifos[c].acquire(ObjectFifoPort.Produce, 1) # 4608/4 = 1152
                        # kernel runs here
                        realJoinFifos[c].release(ObjectFifoPort.Produce, 1)
                        add2mainFIFOs[c].release(ObjectFifoPort.Consume, 1)
                    main_in_fifos[c][0].release(ObjectFifoPort.Consume, 1)

            @core(imag_tiles[c])
            def core_body():
                for _ in range_(ITER_KERNEL):
                    visC = main_in_fifos[c][1].acquire(ObjectFifoPort.Consume, 1) # 4610
                    for _ in range_(BSIZE):
                        inputs = add2mainFIFOs[c].acquire(ObjectFifoPort.Consume, 1) # 4608
                        output = imagJoinFifos[c].acquire(ObjectFifoPort.Produce, 1) # 4608/4 = 1152
                        # kernel runs here
                        imagJoinFifos[c].release(ObjectFifoPort.Produce, 1)
                        add2mainFIFOs[c].release(ObjectFifoPort.Consume, 1)
                    main_in_fifos[c][1].release(ObjectFifoPort.Consume, 1)
        @core(sub_tile)
        def core_body():
            for _ in range_(ITER_KERNEL):
                for _ in range_(BSIZE):
                    inputR = real_joined_fifo.acquire(ObjectFifoPort.Consume, 1) # 1152
                    inputC = imag_joined_fifo.acquire(ObjectFifoPort.Consume, 1) # 1152
                    output = of_sub.acquire(ObjectFifoPort.Produce, 1) # 1152
                    # kernel runs here
                    of_sub.release(ObjectFifoPort.Produce, 1)                    
                    real_joined_fifo.release(ObjectFifoPort.Consume, 1)
                    imag_joined_fifo.release(ObjectFifoPort.Consume, 1)
        
        @core(mean_tile) # output
        def core_body():
            for _ in range_(ITER_KERNEL):
                for _ in range_(ITERS):
                    output = of_out.acquire(ObjectFifoPort.Produce, 1) # 2
                    for _ in range_(MIN_SIZE):
                        inputs = of_sub.acquire(ObjectFifoPort.Consume, 1) # 1152
                        # kernel runs here
                        of_sub.release(ObjectFifoPort.Consume, 1)
