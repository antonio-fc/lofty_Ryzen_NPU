...
def loafty(opts):
    ...
    def device_body():
        ...
        # Inputs
        of_in_factorA = object_fifo("in0", st[1], main_compute_tilesA + main_compute_tilesB, 2, factor_ty)
        ...
        for i in COLS:
            for j in ROWS:
                ...
                factorFIFOs = [of_in_factorA, of_in_factorA]
                for c in range(2):
        ...
        tiles_to_trace = main_compute_tilesA + [mean_tile]
        if enableTrace:
        ...
        def sequence(factorA, factorB, mainA, mainB, output):
            if enableTrace:
                trace_utils.configure_packet_tracing_aie2(tiles_to_trace, trace_shim_tile, opts.trace_size, OUT_SIZE*ITERS)
            npu_dma_memcpy_nd(metadata=of_in_factorA, bd_id=1, mem=factorA, sizes=[1, 1, 1, FACTOR_SIZE])
