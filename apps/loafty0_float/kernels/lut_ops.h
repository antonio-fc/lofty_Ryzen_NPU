...
aie::vector<uint32, VEC_SIZE> v16float_to_v16uint(aie::vector<float, VEC_SIZE> input) {
    ...
    aie::vector<uint32, VEC_SIZE> bitmap0 = aie::broadcast<uint32, VEC_SIZE>(0x007fffff);
    aie::vector<uint32, VEC_SIZE> mantissa = aie::bit_and(input.cast_to<uint32>(), bitmap0);
    ...
    aie::vector<uint32, VEC_SIZE> bitmap1 = aie::broadcast<uint32, VEC_SIZE>(0x7f800000);
    aie::vector<uint32, VEC_SIZE> exponent_unshifted = aie::bit_and(input.cast_to<uint32>(), bitmap1);
    aie::vector<uint32, VEC_SIZE> exponent_biased = aie::downshift(exponent_unshifted, 23);
    // 3. Substract the bias in the exponent (bias = 127) using a broadcast vector
    aie::vector<int32, VEC_SIZE> exponent_signed = aie::sub(exponent_biased.cast_to<int32>(), 127);
    // 4. Save the absolute value of the exponent in an uint32 vector
    aie::vector<int32, VEC_SIZE> exponent_abs = aie::abs(exponent_signed);
    // 5. Save the number of left and right shift for each element 
    aie::vector<uint32, VEC_SIZE> left = aie::downshift(aie::add(exponent_signed, exponent_abs), 1).cast_to<uint32>();
    aie::vector<uint32, VEC_SIZE> right = aie::downshift(aie::add(aie::neg(exponent_signed), exponent_abs), 1).cast_to<uint32>();
    // 6. Shift the mantissa vector according to the sign of the exponent (bitmap) and the exponent (vector)
    aie::vector<uint32, VEC_SIZE> index = mantissa;
...
__attribute__((always_inline))  aie::vector<float, VEC_SIZE> getSinFloat(aie::vector<float, VEC_SIZE> x) {
...
__attribute__((always_inline))  aie::vector<float, VEC_SIZE> getCosFloat(aie::vector<float, VEC_SIZE> x) {
