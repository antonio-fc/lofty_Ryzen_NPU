...
def main(opts):
    ...
    gg = np.arange(0, 32, 0.03125, dtype=INOUT0_DATATYPE)
    # inout2b = np.tile(gg, 9) # v
    inout2b = gg
    for x in range(2, 10):
        inout2b = np.append(inout2b, gg*x)
    ...
    # print("\nlmn Input: ")
    # for x in inout3.reshape(BSIZE, 3):
    #     print(f"l: {x[0]}, m: {x[1]}, n: {x[2]}")
    ...
    for i in range(num_iter):
        ...
        if i == num_iter-1:
            ...
            for bb in output_buffer.reshape(18, 1024):
                print(bb)
