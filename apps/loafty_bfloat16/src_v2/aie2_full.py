...
def loafty(opts):
    ...
    def device_body():
        ...
        input_half_ty = np.ndarray[(INPUT_SIZE//2,), dtype]
        ...
        of_in_mainA = object_fifo("in1", ST[1], MT[0], [1, 1], input_ty)
        # of_in_mainB = object_fifo("in2", ST[1], core_, [1, 1], input_ty)

        cores = [core_3, core_2]
        dist_fifos = [object_fifo(f"of_in_main{t}", MT[0], cores[t], [1, 1], input_half_ty) for t in range(2)]
        object_fifo_link(of_in_mainA, dist_fifos, [], [0, INPUT_SIZE//2])


        join_fifos = [object_fifo(f"of_in_main{t}_", cores[t], MT[0], [1, 1], input_half_ty) for t in range(2)]
        of_gg = object_fifo("gg", MT[0], core_, [1, 1], input_ty)
        object_fifo_link(join_fifos, of_gg, [0, INPUT_SIZE], [])


        of_out = object_fifo("out", core_, ST[2], [1, 1], input_ty)
        # Benched core definition
        @core(core_2)
        def core_body():
            for _ in range_(ITER_KERNEL):
                input1 = dist_fifos[1].acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                output = join_fifos[1].acquire(ObjectFifoPort.Produce, 1) # 4608
                # for _ in range_(ITERS):
                #     kernels['sub'](input1, input2, output, INPUT_SIZE)
                join_fifos[1].release(ObjectFifoPort.Produce, 1)
                dist_fifos[1].release(ObjectFifoPort.Consume, 1)

        @core(core_3)
        def core_body():
            for _ in range_(ITER_KERNEL):
                input1 = dist_fifos[0].acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                output = join_fifos[0].acquire(ObjectFifoPort.Produce, 1) # 4608
                # for _ in range_(ITERS):
                #     kernels['sub'](input1, input2, output, INPUT_SIZE)
                join_fifos[0].release(ObjectFifoPort.Produce, 1)
                dist_fifos[0].release(ObjectFifoPort.Consume, 1)
        ...
        def core_body():
            for _ in range_(ITER_KERNEL):
                input1 = of_gg.acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
