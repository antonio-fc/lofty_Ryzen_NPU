...
def loafty(opts):
    ...
    def device_body():
        ...
        # Inputs
        of_in_factor = object_fifo("in0", ST[1], core_, 2, factor_ty)
        of_in_mainA = object_fifo("in1", ST[0], core_, 2, main_ct_ty)       
        of_in_mainB = object_fifo("in2", ST[2], core_off, 2, join_ty)  
        ...
        def core_body():
            for _ in range_(ITER_KERNEL):
                f = 1.0
                input1 = of_in_mainA.acquire(ObjectFifoPort.Consume, 5) # 2 + 4608 = 4610
                input2 = of_in_factor.acquire(ObjectFifoPort.Consume, 1) # 2 + 4608 = 4610
                output = of_out.acquire(ObjectFifoPort.Produce, 1) # 4608
                for _ in range_(ITERS):
                    kernels['main'](f, input2, input1[0], input1[1], input1[2], input1[3], input1[4], output, MAIN_SIZE)
                of_out.release(ObjectFifoPort.Produce, 1)
                of_in_factor.release(ObjectFifoPort.Consume, 1)
                of_in_mainA.release(ObjectFifoPort.Consume, 5)
